<script lang="ts">
	...
	export let value: string = 'survival'
	...
	function toggle() {
		open = !open
		if (open) focusedIndex = gamemodes.findIndex(g => g.value === value)
	}
	...
	function onKeyDown(e: KeyboardEvent) {
		...
		switch (e.key) {
			case 'ArrowDown':
				focusedIndex = (focusedIndex + 1) % gamemodes.length
				e.preventDefault()
				break
			case 'ArrowUp':
				focusedIndex = (focusedIndex - 1 + gamemodes.length) % gamemodes.length
				e.preventDefault()
				break
			case 'Enter':
			case ' ':
				if (focusedIndex >= 0) select(gamemodes[focusedIndex])
				e.preventDefault()
				break
			case 'Escape':
				close()
	...
	function onFocus() {
		open = true
		focusedIndex = gamemodes.findIndex(g => g.value === value)
	}
...
<div class="flex flex-col w-full">
	...
	>
		<span
			class="w-full text-left text-base text-gray-900 select-none">{gamemodes.find(g => g.value === value)?.label}</span>
		<span
